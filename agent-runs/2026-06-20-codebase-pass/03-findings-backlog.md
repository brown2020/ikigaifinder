...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | P2 | Dead code | Queued | Legacy generation constants/types | Remove unused `IKIGAI_SYSTEMPROMPT`, exported `questions`, and `SurveyQuestion` while preserving active `IKIGAI_SYSTEMPROMPT2` and `STEPPER_QUESTIONS_JSON`. | `rg` shows only `IKIGAI_SYSTEMPROMPT2` is imported by `src/lib/generateIkigai.ts`; `STEPPER_QUESTIONS_JSON` is imported by `ikigaiService` and `IkigaiStepperForm`; legacy `questions` only imports `SurveyQuestion` within `src/constants/questions.ts`. | Medium: stale public surface confuses future generation/questionnaire work. | Small | `rg`, `npm run lint`, `npm test`, `npm run build` | Execute T-004 now. |
