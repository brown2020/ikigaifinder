...
import { collection, deleteDoc, doc, DocumentReference, getDocs } from "firebase/firestore";
...
export default function ProfileDeleted({ isOpen, closeModal, setToast }: ProfileDeletedProps) {
  ...
  const handleDeleteIkigaiProfileData = async (): Promise<string | undefined> => {
    ...
    const ikigaiRefs = [doc(db, `/ikigaiUsers/${uid}/ikigai/main`), doc(db, `/ikigaiUsers/${uid}`)];

    try {
      const subCollectionSnapshot = await getDocs(coverCollectionRef);
      await Promise.all([...subCollectionSnapshot.docs.map(doc => deleteDoc(doc.ref)), deleteDocs([mainIkigaiProfileRef, ...ikigaiRefs])]);
  ...
  const handleDeleteProfileData = async () => {
    const profileRefs = [doc(db, `/ikigaiUsers/${uid}/settings/profile`), doc(db, `/ikigaiUsers/${uid}`)];
  ...
  const deleteConfirmIkigaiProfile = async () => {
    ...
    try {
      ...
      if (removeIkigaiData) {
        ...
        setToast({ show: true, type: "success", message: "Ikigai profile successfully deleted!" });
    ...
    <div>
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div ref={modalRef} className="bg-white rounded-lg p-5 w-full xs:max-w-md max-w-72">
